...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
		...
		for (i = 0; i < conf_array.size; i++) {
			if (conf_array.reg_setting->reg_addr == 0x6004) {
				while (conf_array.reg_setting->reg_addr != 0xEEEE) {
					...
					if (rc < 0) {
						pr_err("%s: %d write conf_array fail, "
							"addr=0x%x data=0x%x datatype=0x%d\n",
							__func__, __LINE__,
							conf_array.reg_setting->reg_addr,
							conf_array.reg_setting->reg_data,
							conf_array.reg_setting->reg_data_type);
				...
				if (conf_array.reg_setting->reg_addr == 0xEEEE
				...
				for (array_index = 0; array_index < array_length; array_index++) {
					rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_seq_ex(
						s_ctrl->sensor_i2c_client,
						reg_setting_ex[array_index].reg_addr,
						reg_setting_ex[array_index].reg_data,
						reg_setting_ex[array_index].reg_data_size);
					if (rc < 0) {
				...
				if (rc < 0) {
					pr_err("%s : %d write conf_array fail, addr=0x%x data=0x%x datatype=0x%d\n",
						__func__, __LINE__, conf_array.reg_setting->reg_addr,
						conf_array.reg_setting->reg_data, conf_array.reg_setting->reg_data_type);
					break;
				}
			}
			conf_array.reg_setting++;
	...
	case CFG_SENSOR_GET_OTP: {
		...
		rc = s_ctrl->sensor_get_otp(s_ctrl,
				cdata->sensor_otp_params_ptr);
		if (rc < 0) {
