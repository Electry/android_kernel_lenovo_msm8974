...
static int msm_eeprom_fancymaker_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	for (data_index = sensor_3a_begin_offset;
			data_index <= sensor_3a_end_offset;
			data_index++) {
		data_3a_sum = data_3a_sum + e_ctrl->cal_data.mapdata[data_index];
	}

	CDBG("%s: data_3a_sum = 0x%x  temp_data=0x%x 0x%x\n",
		__func__, data_3a_sum,
		(*(e_ctrl->cal_data.mapdata+sensor_3a_checksum_hi_offset)),
		(*(e_ctrl->cal_data.mapdata+sensor_3a_checksum_low_offset)));

	if ((data_3a_sum&0xffff)
			== (((*(e_ctrl->cal_data.mapdata+sensor_3a_checksum_hi_offset))<<8)
				| (*(e_ctrl->cal_data.mapdata+sensor_3a_checksum_low_offset)))) {
	...
	CDBG("%s: data_ois_sum = 0x%x temp_data=0x%x 0x%x\n",
		__func__,
		data_ois_sum,(*(e_ctrl->cal_data.mapdata+ois_checksum_hi_offset)),
		(*(e_ctrl->cal_data.mapdata+ois_checksum_low_offset)));

	if ((data_ois_sum&0xffff)
			== (((*(e_ctrl->cal_data.mapdata+ois_checksum_hi_offset))<<8)
				| (*(e_ctrl->cal_data.mapdata+ois_checksum_low_offset)))) {
